...
pve5x(){
    echo -n "Your OS is Proxmox VE $pve ,you will install OpenMediaVault 4"
...
nginx pm-utils wpasupplicant samba-vfs-modules python3-pyudev python3-natsort jq ntp python3-netifaces python3-lxml resolvconf
    ...
    wget http://packages.openmediavault.org/public/pool/main/o/openmediavault/openmediavault_4.1.35-1_all.deb
...
pve6x(){
    echo -n "Your OS is Proxmox VE $pve ,you will install OpenMediaVault 5"
    sleep 2
...
salt-minion samba samba-vfs-modules sdparm sshpass sudo tdb-tools uuid wpasupplicant wsdd xmlstarlet
    ...
    wget -c http://packages.openmediavault.org/public/pool/main/o/openmediavault/openmediavault_5.5.9-1_all.deb
    ...
    apt-get -y -f install
    dpkg --force-all -i openmediavault_5.5.9-1_all.deb
    echo "Installation Complete, now init the OpenMediaVault 5!"
    rm -rf ./omvtmp
...
while [ true ] 
    do
        ...
        echo -n "Your OS is Proxmox VE $pve , you will install OpenMediaVault, continue?(y/n)"
        read x
        case "$x" in
        y | yes ) 
            echo -n "Your OS is Proxmox VE $pve , you will install OpenMediaVault"
            ...
            if [ $ver = "5.3" ];then
                pve5x
                echo "Installation Complete! Go to http://your_native_ip to enjoy your OpenMediaVault 4!"
                exit
            elif [ $ver = "5.4" ];then
                pve5x
                echo "Installation Complete! Go to http://your_native_ip to enjoy your OpenMediaVault 4!"
                ...
                pve6x
                echo "Installation Complete! Go to http://your_native_ip to enjoy your OpenMediaVault 5!"
                exit
            elif [ $ver = "6.1" ];then
                pve6x
                echo "Installation Complete! Go to http://your_native_ip to enjoy your OpenMediaVault 5!"
                ...
                echo "Your proxmox ve version is not 5.x/6.x, it may harm your system, now quit."
                break
            fi
        ;;
